...
import { Trophy, Clock, DollarSign, Target, Car, Shield, Zap } from 'lucide-react';
...
const CAR_COLORS = ['#FF0000', '#0000FF', '#FFFF00', '#FF8000', '#800080', '#008000'];
...
export default function GTAGame() {
  ...
  useEffect(() => {
    ...
    const checkPlayStatus = async () => {
      const status = await MiniGameEngine.canPlayGame('gtaV1', user.id.toString());
          ...
          case 'e':
            ...
            if (prev.onFoot) {
              ...
              if (nearCar) {
                setCars(prevCars => 
                  prevCars.map(car => 
                    car.id === nearCar.id ? { ...car, stolen: true, active: false } : car
                  )
                );
  ...
  useEffect(() => {
    ...
    if (gameState === 'playing') {
      ...
      // Wanted level
      ctx.fillStyle = '#FF0000';
      ctx.fillText(`Wanted: ${'★'.repeat(wantedLevel)}`, 20, CANVAS_HEIGHT - 15);
    }
    ...
    <Card className="w-full max-w-4xl mx-auto bg-gradient-to-br from-gray-900 via-red-900 to-black text-white">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-400">{'★'.repeat(wantedLevel)}</div>
